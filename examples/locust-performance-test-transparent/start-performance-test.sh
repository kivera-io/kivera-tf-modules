...
[[ $CLEANUP != false ]] && echo "Skipping cleanup for proxy" && trap "terraform -chdir=$proxy_dir destroy --auto-approve" EXIT
...
if [[ $KIVERA_TRANS_MODE == true ]]; then
    timeout=$((SECONDS + 600))
    while true; do
        ...
        if [[ $one_healthy == true ]]; then
            ...
            echo "Waiting for healthy proxy instance..."
            sleep 5
        fi
    done
fi
