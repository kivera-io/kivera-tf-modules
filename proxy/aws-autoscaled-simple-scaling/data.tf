...
locals {
  ...
  redis_endpoint                     = local.redis_enabled && var.serverless_cache ? try(aws_elasticache_serverless_cache.redis[0].endpoint[0].address, "") : try(aws_elasticache_replication_group.redis[0].configuration_endpoint_address, "")
  ...
  cache_cluster_name                 = var.cache_enabled && var.serverless_cache ? try(aws_elasticache_serverless_cache.redis[0].name, "") : try(aws_elasticache_replication_group.redis[0].id, "")
}
