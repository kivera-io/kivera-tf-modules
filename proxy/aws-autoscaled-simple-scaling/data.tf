...
locals {
  ...
  cache_cluster_name                 = local.redis_enabled ? (var.serverless_cache ? aws_elasticache_serverless_cache.redis[0].name : aws_elasticache_replication_group.redis[0].id) : ""
}
